...
def showBanner():
	...
	print ("Comrade, Your MySQL is now OurSQL")
	print("(v{})\n".format(getVersion()))
...
def connect(host, user, password, port, timeout):
	...
	try:
		...
		pymysql.connect(host=host, port=port, user=user, password=password, connect_timeout=timeout)
		success = True
	...
	try:
		...
		for host in hosts:
			...
			if not connectable or lost_connection:
				showFailure("Unable to connect to {}:{}".format(host, port))
				connectable = True
				lost_connection = False
				continue
			if not allowed:
				showFailure("Host {}:{} doesn't allow us to access their MySQL!".format(host, port))
				allowed = True
				continue
			if unknown:
				showFailure("Unknown host {}!".format(host))
				unknown = False
				continue
			...
			for user in users:
				...
				if success:
					...
					showFailure("Unable to find password for user {}".format(user))
		exitScript()
